...
//        mContext.startProgressSpinner();
    ...
    private void drawAmPm(Canvas canvas, Paint p) {
        ...
        p.setTextSize(AMPM_TEXT_SIZE);
        //p.setTypeface(mBold); //yanghong remove the bold style on Sep.26,2014
        p.setAntiAlias(true);
        p.setTextAlign(Paint.Align.RIGHT);
    ...
    private StaticLayout getEventLayout(StaticLayout[] layouts, int i, Event event, Paint paint,
        ...
        if (layout == null || r.width() != layout.getWidth()) {
            ...
            if (event.title != null) {
                ...
                //bob.setSpan(new StyleSpan(android.graphics.Typeface.BOLD), 0, bob.length(), 0); //yanghong remove the bold style.
                bob.append(' ');
            }
            if (event.location != null) {
                //yanghong add on Sep.26,2014
                bob.append('\n');
                bob.append(mResources.getString(R.string.where_label)+":");
                //yanghong add end
    ...
    private void drawEventText(StaticLayout eventLayout, Rect rect, Canvas canvas, int top,
        ...
        for (int i = 0; i < lineCount; i++) {
            ...
            if (lineBottom <= height) {
                ...
                //yanghong add for PROD103335180
                //if only one line, the totalLineHeight shouldn't be 0.
                if(lineCount == 1){
                    totalLineHeight = lineBottom;
                }
                //yanghong add end
