...
import com.ktouch.calendarcommon.DateException;
import com.ktouch.calendarcommon.EventRecurrence;
import com.ktouch.calendarcommon.RecurrenceProcessor;
import com.ktouch.calendarcommon.RecurrenceSet;
...
public class EditEventHelper {
    ...
    public EditEventHelper(Context context) {
        /*TIANYURD:zhanggh 20150113 modify for PROD103296184 begin*/
        //mService = ((AbstractCalendarActivity)context).getAsyncQueryService();
        mService = new AsyncQueryService(context);
        mContext = context;
        /*TIANYURD:zhanggh 20150113 modify for PROD103296184 end*/
    }
    ...
    public boolean saveEvent(CalendarEventModel model, CalendarEventModel originalModel,
        ...
        Log.e("zys", "112------------- eventId = " + eventId + ", eventIdIndex = " + eventIdIndex);
        //saveKCalendarEvent(model, eventId, newEvent);

        return true;
    }

    public boolean saveKCalendarEvent(CalendarEventModel model, long eventId, boolean newEvent) {

        ContentValues values = new ContentValues();
        int recurrenceIndex = model.mRecurrenceIndex;
        int markAs = model.mMarkAs;
//        values.put(KCalendarProvider.Columns.RECURRENCE_INDEX, recurrenceIndex);
//        values.put(KCalendarProvider.Columns.MARK_AS, markAs);

        Uri uri = ContentUris.withAppendedId(KCalendarProvider.Columns.CONTENT_URI, eventId);
        ArrayList<ContentProviderOperation> ops = new ArrayList<ContentProviderOperation>();
        ...
        if (newEvent) {
            b = ContentProviderOperation.newInsert(
                    KCalendarProvider.Columns.CONTENT_URI).withValues(values);
        } else {
            b = ContentProviderOperation.newUpdate(uri).withValues(values);
        }
        ops.add(b.build());

        mService.startBatch(mService.getNextToken(), null, KCalendarProvider.AUTHORITY, ops,
                Utils.UNDO_DELAY);
        return true;
    }

    public static int[] queryKCalendar(Context context, long eventId) {
...
//                int recurrenceIndexColumn = c.getColumnIndex(KCalendarProvider.Columns.RECURRENCE_INDEX);
//                int markAsColumn = c.getColumnIndex(KCalendarProvider.Columns.MARK_AS);
                if (c.getLong(id) == eventId) {
//                    recurrenceIndex = c.getInt(recurrenceIndexColumn);
//                    markAs = c.getInt(markAsColumn);
        ...
        return new int[]{recurrenceIndex, markAs};
