...
import com.ktouch.kcalendar.LunarUtils;
...
public class MonthWeekEventsView extends SimpleWeekView {
    ...
    //zhang-yi start
    public static final String VIEW_PARAMS_MODE = "mode";
    private int mMode;
    //zhang-yi end
    ...
    private static int LUNAR_Y_OFFSET = 5;  // More bigger, the gap will be smaller.
    ...
    protected void loadColors(Context context) {
        ...
        mMonthNumColor = res.getColor(R.color.primary_text_black_color);//zhang-yi
        mMonthNumOtherColor = res.getColor(R.color.primary_text_gray_color);//zhang-yi
        ...
        mClickedDayColor = res.getColor(R.color.click_day_color);//zhang-yi
    ...
    public void setWeekParams(HashMap<String, Integer> params, String tz) {
        ...
        if (params.containsKey(VIEW_PARAMS_MODE)) {
            mMode = params.get(VIEW_PARAMS_MODE);
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        if(Utils.mWeekLineHeight==0){
            Utils.mWeekLineHeight = mHeight;
        }
        Log.d(TAG, "-=-=-= onDraw");
        drawBackground(canvas);
        drawWeekNums(canvas);
//        drawDaySeparators(canvas);//zhang-yi
...
//        }
    ...
    private int getDayLeftPositionOffset(String dayNum, Paint paint) {
        int effectiveWidth = mWidth;
        int offset = 0;
        int xOffset = 0;
        if (mShowWeekNum) {
        ...
        offset = (effectiveWidth / 7 - rect.width()) / 2;
        return offset;
    }
    ...
    private void drawClick(Canvas canvas) {
        if (mClickedDayIndex != -1) {
            ...
            if (mHasToday && todayIndex == mClickedDayIndex) {
                p.setColor(mMonthNumTodayColor);
            } else {
                alpha = 50;
            }
...
//            r.bottom = mHeight;
//            canvas.drawRect(r, p);
            int dayWitdh = computeDayLeftPosition(mClickedDayIndex + 1) - computeDayLeftPosition(mClickedDayIndex);
            float cx = computeDayLeftPosition(mClickedDayIndex) + dayWitdh/ 2;
            float cy = mHeight / 2;
            float radius = getResources().getDimension(R.dimen.day_selected_circle_radius);
    ...
    private void setMonthPainColor(boolean isFocusMonth){
    ...
    protected void drawWeekNums(Canvas canvas) {
        Resources res = getResources();//zhang-yi
        int y;
        ...
        setMonthPainColor(isFocusMonth);
...
//                        mMonthNumPaint.setColor(mMonthNumColor);
            ...
            //zhang-yi start
            x = computeDayLeftPosition(i - offset) - getDayLeftPositionOffset(mDayNumbers[i], mMonthNumPaint);// - (SIDE_PADDING_MONTH_NUMBER);
            mMonthNumPaint.setTextSize((float)res.getDimension(R.dimen.middle_text_size));
            //zhang-yi end
            ...
            String key = year + "-" + month + "-" + monthDay;
            if (VacationUtils.mVacationMap!=null && VacationUtils.mVacationMap.containsKey(key)) {
                int holidayX = computeDayLeftPosition(i) + SIDE_PADDING_MONTH_NUMBER;
                int holidayY = TOP_PADDING_MONTH_NUMBER;
                Paint holidayPaint = new Paint();
                holidayPaint.setTextSize(res.getDimension(R.dimen.holiday_sign_size));
                String signText = "";
                switch (VacationUtils.mVacationMap.get(key).mState) {
                    case 1:
                    case 3:
                        signText = res.getText(R.string.holiday_sign_text).toString();
                        break;
                    case 2:
                        signText = res.getText(R.string.workday_sign_text).toString();
                        break;
                    default:
                        break;
                }
                canvas.drawText(signText, holidayX, holidayY, holidayPaint);
            }
            //zhang-yi end
            ...
            if (LunarUtils.showLunar(getContext())) {
                ...
                if (infos.size() > 0) {
                    ...
                    mMonthNumPaint.setTextSize(res.getDimension(R.dimen.mini_mini_text_size));//zhang-yi
//                    Resources res = getResources();//zhang-yi
                    ...
                    for (int index = 0; index < infos.size(); index++) {
                        ...
                        if (mOrientation == Configuration.ORIENTATION_LANDSCAPE) {
                            ...
                            infoY = y + (mMonthNumHeight) * num;
                        } else {
                            infoX = x;
                            infoY = y + (mMonthNumHeight) * (num + 1);
                        }
                        canvas.drawText(info, computeDayLeftPosition(i - offset) - getDayLeftPositionOffset(info, mMonthNumPaint), infoY-LUNAR_Y_OFFSET, mMonthNumPaint);
                        num = num + 1;
                    }
    ...
    protected void drawEvents(Canvas canvas) {
        ...
        for (ArrayList<Event> eventDay : mEvents) {
            ...
            //zhang-yi start
            int dayWitdh = computeDayLeftPosition(day + 1) - computeDayLeftPosition(day);
            float cx = computeDayLeftPosition(day) + dayWitdh/ 2;
            float cy = mHeight - getResources().getDimension(R.dimen.event_sign_bottom_padding);
            float radius = getResources().getDimension(R.dimen.event_sign_radius);
            Paint eventPaint = new Paint();
            eventPaint.setColor(getResources().getColor(R.color.primary_event_sign_color));
            canvas.drawCircle(cx, cy, radius, eventPaint);
...
//            }
    ...
    public int getDayIndexFromLocation(float x) {
        Log.d(TAG, "-=-=-= getDayIndexFromLocation mShowWeekNum = " + mShowWeekNum
                + ", SPACING_WEEK_NUMBER = " + SPACING_WEEK_NUMBER
                + ", mPadding = " + mPadding + ", mWidth = " + mWidth
                + ", mNumDays = " + mNumDays);
    ...
    public void setClickedDay(float xLocation) {
        Log.d(TAG, "-=-=-= setClickedDay xLocation = "+xLocation);
        mClickedDayIndex = getDayIndexFromLocation(xLocation);
        Log.d(TAG, "-=-=-= setClickedDay mClickedDayIndex = "+mClickedDayIndex);
        invalidate();
    }

    //zhang-yi start
    public void setClickedDay(int weekDay) {
        setClickedDay((float)computeDayLeftPosition(weekDay) + 5.0f);
    }
    //zhang-yi end
