...
import com.ktouch.kcalendar.agenda.AgendaWindowAdapter.DayAdapterInfo;
...
public class AgendaByDayAdapter extends BaseAdapter {
    ...
    public AgendaByDayAdapter(Context context) {
        mContext = context;
        mAgendaAdapter = new AgendaAdapter(context, R.layout.agenda_item_with_icon);
    ...
    public AgendaByDayAdapter(Context context, int windowType) {
        mContext = context;
        if (windowType == AgendaListView.WINDOW_TYPE_SEARCH) {
            mAgendaAdapter = new AgendaAdapter(context, R.layout.agenda_item_with_icon_for_search);
        } else {
            mAgendaAdapter = new AgendaAdapter(context, R.layout.agenda_item_with_icon);
        }
        mInflater = (LayoutInflater) mContext.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (row.mType == TYPE_DAY) {
            ...
            if (holder == null) {
                ...
                agendaDayView = mInflater.inflate(R.layout.agenda_day_title, parent, false);
            ...
            if (row.mDay == mTodayJulianDay) {
                holder.dateView.setTextColor(mContext.getResources().getColor(R.color.primary_text_red_color));
            } else {
                holder.dateView.setTextColor(mContext.getResources().getColor(R.color.primary_text_black_color));
            }

            if (mWindowType != AgendaListView.WINDOW_TYPE_YEAR) {
...
//                itemView.setBackgroundResource(R.drawable.agenda_item_bg_primary);
    ...
    private boolean mHaveEventToday = false;
    ...
    public void calculateDays(DayAdapterInfo dayAdapterInfo) {
        ...
        for (int position = 0; cursor.moveToNext(); position++) {
            ...
            if ( mTodayJulianDay > prevStartDay && mTodayJulianDay < startDay) {
                mPosition = position;
            }
            if (allDay) {
        ...
        for (RowInfo ri : rowInfo) {
            if (ri.mDay == mTodayJulianDay) {
                mHaveEventToday = true;
                break;
            }
        }
        if (!mHaveEventToday) {
            // We have to Pass mPosition items because there have mPosition lines include both
            // date and event item.
            mPosition+=mPosition;
            try {
                rowInfo.add(mPosition, new RowInfo(TYPE_DAY, mTodayJulianDay));
            }catch (Exception e){
