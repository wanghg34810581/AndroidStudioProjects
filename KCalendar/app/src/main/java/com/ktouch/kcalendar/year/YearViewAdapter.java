package com.ktouch.kcalendar.year;
...
import com.ktouch.datetimepicker.date.MonthView;
...
public class YearViewAdapter extends BaseAdapter implements View.OnTouchListener {
    ...
    private static final double MONTH_HEADER_HEIGHT_WITHOUT_WEEKDAY = 0.20;
    ...
    private static final double ROW_HEIGHT_WITHOUT_WEEKDAY = (1 - MONTH_HEADER_HEIGHT_WITHOUT_WEEKDAY) / 6;
    ...
    public YearViewAdapter(Context context, int year, int columns) {
        ...
        mSetByDimens = Utils.getConfigBool(mContext, R.bool.set_by_dimens);
        mPadding = mContext.getResources().getDimensionPixelSize(R.dimen.year_view_padding);
        mHeaderSize = mContext.getResources().getDimensionPixelSize(R.dimen.month_list_item_header_height);
        mMonthLabelTextSize = mContext.getResources().getDimensionPixelSize(R.dimen.month_label_size);
        mMonthDayTextSize = mContext.getResources().getDimensionPixelSize(R.dimen.day_number_size);
    }
...
//            linearLayout.setBackgroundColor(mCurrentMonthBgColor);
        ...
        if(mSetByDimens){
            ...
            drawingParams.put(MonthView.CONFIG_EDGE_PADDING, mPadding);
            // add Month title header height
            drawingParams.put(MonthView.CONFIG_HEADER_SIZE, mHeaderSize);
            // add Month label font size
            drawingParams.put(MonthView.CONFIG_MONTH_LABEL_TEXT_SIZE,
                    mMonthLabelTextSize);
            // add text size for month days
            drawingParams.put(MonthView.CONFIG_MONTH_DAY_TEXT_SIZE, mMonthDayTextSize);
        }else {
            // padding in pixels ; applies to the left and right edges
        ...
        //hunan 2016-01-13 PROD104138833 adjust the row height
        // add horizontal padding
        if(MonthView.SHOW_WEEK_DAY) {
            drawingParams.put(MonthView.CONFIG_MONTH_ROW_HEIGHT,
                    (int) (mMonthHeight * ROW_HEIGHT - 3));
        } else {
            drawingParams.put(MonthView.CONFIG_MONTH_ROW_HEIGHT,
                    (int) (mMonthHeight * ROW_HEIGHT_WITHOUT_WEEKDAY - 3));
        }
...
//            }
    ...
    private final Runnable mPerformClick = new Runnable() {
        ...
        public void run() {
            ...
            time.setToNow();
            todayCalendar.setTimeInMillis(time.toMillis(false));
            time.set(Utils.CLICKED_DAY, mClickedView.getId(), mYear);   // set(day, month, year)
            if(!Utils.SELECTED_DAY_IS_DEFAULT && todayCalendar!=null && mYear==todayCalendar.get(Calendar.YEAR)
                    && mClickedView.getId()==todayCalendar.get(Calendar.MONTH)) {
                time.set(todayCalendar.get(Calendar.DAY_OF_MONTH), mClickedView.getId(), mYear);
            }
