...
import com.ktouch.kcalendar.CalendarUtils.TimeZoneUtils;
...
public class Utils {
    ...
    public static final int YEAR_MAX = 2037; //wangqin fixed it for ending date to 2037 at PROD103829338
    ...
    public static ArrayList<Long> mDatesArray = new ArrayList<Long>();

    public static int CLICKED_DAY = 1;

    public static int mWeekLines = 0;
    public static int mWeekLineHeight = 0;

    public static final int DAYS_PER_WEEK = 7;

    public static boolean mCanRefresh = false;

    public static final boolean SELECTED_DAY_IS_DEFAULT = false;

    public static boolean mYearToMonth;
    ...
    **/
    ...
    public static Spannable extendedLinkify(String text, boolean lastDitchGeo) {
         ...
         * run first because every invacation removes all previous URLSpan annotations.
         *
...
//    }
    ...
    public static String getDateWithWeek(Context context, long millis) {
        ...
        synchronized (TimeZone.class) {
            TimeZone.setDefault(TimeZone.getTimeZone(Utils.getTimeZone(context, null)));
    ...
    public static String getWholeDateFormat(Context context, long millis) {
        int flags = DateUtils.FORMAT_SHOW_DATE
                | DateUtils.FORMAT_SHOW_YEAR
                | DateUtils.FORMAT_SHOW_WEEKDAY
                | DateUtils.FORMAT_ABBREV_MONTH
    ...
    public static String getYearAndMonthFormat(Context context, long millis) {
        int flags = DateUtils.FORMAT_SHOW_DATE
    ...
    public static String getWholeTimeFormat(Context context, long millis) {
    ...
    public static void setDateAndTime(Context context, TextView view, long millis) {
        int flags = DateUtils.FORMAT_SHOW_DATE
                | DateUtils.FORMAT_SHOW_YEAR
                | DateUtils.FORMAT_ABBREV_MONTH;
        ...
        if (view != null) {
            view.setText(getWholeDateFormat(context, millis)+context.getResources().getString(R.string.space)+getWholeTimeFormat(context, millis));
        }
    }
    ...
    public static void setTime(Context context, TextView view, long millis) {
        ...
        synchronized (TimeZone.class) {
            TimeZone.setDefault(TimeZone.getTimeZone(Utils.getTimeZone(context, null)));
            timeString = DateUtils.formatDateTime(context, millis, flags);
            TimeZone.setDefault(null);
        }
        if (view != null) {
    ...
    public static long calculateDays(long start){
        Calendar endCalendar = Calendar.getInstance();
        endCalendar.set(YEAR_MAX, MONTH_MAX, DAY_MAX);
        long end = endCalendar.getTimeInMillis();
        long diff = end-start;
        return diff/DAY_IN_MILLIS;
    }

    public static int calculateMonths(long start){
        Calendar startCalendar = Calendar.getInstance();
        startCalendar.setTimeInMillis(start);
        int year = YEAR_MAX-startCalendar.get(Calendar.YEAR);
        int month = startCalendar.get(Calendar.MONTH);
        int monthValues = 12-month+year*12;
        return monthValues;
    }

    public static int calculateYears(long start){
        Calendar startCalendar = Calendar.getInstance();
        startCalendar.setTimeInMillis(start);
    ...
    public static int[] loadDrawableArray(Resources r, int drawableStringArray) {
        TypedArray ar = r.obtainTypedArray(drawableStringArray);
        int len = ar.length();
    ...
    public static int getLineCount(Context context, Calendar calendar, int viewType) {
        ...
        if (DEBUG) {
            ...
            android.util.Log.d(TAG, "-=-=-= dayCount / daysPerWeek = " + dayCount / DAYS_PER_WEEK);
            android.util.Log.d(TAG, "-=-=-= dayCount % daysPerWeek = " + dayCount % DAYS_PER_WEEK);
        ...
        int noFirstLineDayCount = dayCount - (DAYS_PER_WEEK - calendar.get(Calendar.DAY_OF_WEEK) + firstDayOfWeek);
        //last +1 is first line
        int lineCount = noFirstLineDayCount / DAYS_PER_WEEK + ((noFirstLineDayCount % DAYS_PER_WEEK > 0) ? 1 : 0) + 1;
        return lineCount;
    }

    public static Time getTomorrow(Time today) {
        Time tomorrow = new Time();
        int year = today.year;
        int month = today.month;
        int day = today.monthDay;
        if (today.getActualMaximum(Time.MONTH_DAY) == day) {
            if (month == 11) {
                year++;
                month = 0;
                day = 1;
