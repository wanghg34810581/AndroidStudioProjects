...
public class SwipeListViewTouchListener implements View.OnTouchListener {
    ...
    //TY zhencc add
    private int springRegion; //反弹的区域
    ...
    public SwipeListViewTouchListener(SwipeListView swipeListView, int swipeFrontView, int swipeBackView) {
        ...
        //TY zhencc add
        springRegion = swipeListView.getContext().getResources().getDimensionPixelSize(R.dimen.swipe_spring_region);
    }
    ...
    private void generateDismissAnimate(final View view, final boolean swap, final boolean swapRight, final int position) {
        ...
        if (opened.get(position)) {
            if (!swap) {
                moveTo = openedRight.get(position) ? (int) (rightOffset) : (int) (-viewWidth + leftOffset);
            }
        } else {
            if (swap) {
                moveTo = swapRight ? (int) (rightOffset) : (int) (-viewWidth + leftOffset);
            }
        }
    ...
    //TY zhencc add for activity back
    boolean haveOpenItem() {
        if (opened != null) {
            int start = mLayoutManager.findFirstVisibleItemPosition();
            int end = mLayoutManager.findLastVisibleItemPosition();
            for (int i = start; i <= end; i++) {
        ...
        switch (MotionEventCompat.getActionMasked(motionEvent)) {
            case MotionEvent.ACTION_DOWN: {
                ...
                for (int i = 0; i < childCount; i++) {
                    ...
                    if (allowSwipe && rect.contains(x, y)) {
                        if (child.findViewById(swipeFrontView) != null) {
                            setParentView(child);
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (swiping && downPosition != ListView.INVALID_POSITION) {
                    if (opened.get(downPosition)) {
                        deltaX += openedRight.get(downPosition) ? rightOffset : -viewWidth + leftOffset;
                    }

                    //TY zhencc add begin
                    int frontViewX = (int) ViewHelper.getX(frontView);

                    if (swipeMode == SwipeListView.SWIPE_MODE_RIGHT) {
                        Log.d(SwipeListView.TAG, "deltaX1 = " + deltaX);
                        Log.d(SwipeListView.TAG, "swipingRight = " + swipingRight);
                        Log.d(SwipeListView.TAG, "frontViewX = " + frontViewX);
                        Log.d(SwipeListView.TAG, "springRegion = " + springRegion);

                        if (swipingRight) {
                            if (deltaX < rightOffset) {  //如果还在正常的滑动区域
                                // 不变
                            } else {
                                deltaX = (deltaX - (rightOffset)) / 5 + (rightOffset);
                            }

                            if (deltaX >= (rightOffset + springRegion)) {   //超过了最大的滑动区域
                                deltaX = rightOffset + springRegion;
                            }
                            Log.d(SwipeListView.TAG, "deltaX2 =++++++++ " + deltaX);
                        } else if (!swipingRight && frontViewX <= 0) {
                            deltaX = 0;
                        }
                    }
                    //TY zhencc add end
    ...
    public void move(float deltaX) {
        ...
        if (opened.get(downPosition)) {
            posX += openedRight.get(downPosition) ? -rightOffset : viewWidth - leftOffset;
        }
